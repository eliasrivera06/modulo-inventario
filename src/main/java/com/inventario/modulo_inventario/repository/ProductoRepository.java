...
public interface ProductoRepository extends JpaRepository<Producto, Long> {
        // Búsqueda para ADMIN (todos los productos)
        ...
        // Búsqueda para EMPLEADO (solo productos disponibles, estado = 1)
        @Query("SELECT p FROM Producto p WHERE " +
                        "p.estado = 1 AND " +
                        "(:keyword IS NULL OR :keyword = '' OR " +
                        "LOWER(p.nombre) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        ...
                        "(:categoria IS NULL OR :categoria = '' OR p.categoria = :categoria) AND " +
                        "(:marca IS NULL OR :marca = '' OR p.marca = :marca)")
        List<Producto> searchDisponibles(@Param("keyword") String keyword,
                        @Param("sede") String sede,
                        @Param("categoria") String categoria,
                        @Param("marca") String marca);
